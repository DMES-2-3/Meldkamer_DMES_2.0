...
class MapUploadController extends BaseController implements IController
{
    ...
    {
        if (!isset($_FILES['file'])) $this->sendError('Missing file', 422);

        $file = $_FILES['file'];
        if ($file['error'] !== UPLOAD_ERR_OK) $this->sendError('Upload error', 422);

        $eventId = $_POST['eventId'] ?? null;
        if (!$eventId) $this->sendError('Missing eventId', 422);

        $event = $this->entityManager->find(Event::class, $eventId);
        if (!$event) $this->sendError('Event not found', 404);

        $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        if ($ext !== 'pdf') $this->sendError('Only PDF files allowed', 422);

        $filename = uniqid('map_', true) . '.pdf';
        $target = $this->uploadsDir . $filename;

        if (!move_uploaded_file($file['tmp_name'], $target)) $this->sendError('File move failed', 500);
